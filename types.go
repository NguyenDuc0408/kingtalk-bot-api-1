...
// File contains information about a file to download from KingTalk
type File struct {
	FileID   string `json:"file_id"`
	FileSize int    `json:"file_size"` // optional
...
// Game is a game within KingTalk
type Game struct {
	Title        string          `json:"title"`
	Description  string          `json:"description"`
...
// InlineQuery is a Query from KingTalk for an inline request.
...
// Error is an error containing extra information returned by the KingTalk Bot API.
type Error struct {
	Code    int
	Message string
	Log     string
